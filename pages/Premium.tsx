...
const Premium: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col pb-32 bg-background-light text-slate-900">
      <header className="bg-black px-6 pt-12 pb-6 flex flex-col gap-4 border-b border-white/10">
        <div className="flex items-center justify-between">
          <span className="text-slate-500 font-black tracking-tighter text-sm">STARDY.AI</span>
          <Logo className="size-8" />
        </div>
        <div className="flex items-center gap-4">
          <button onClick={() => navigate(-1)} className="size-10 flex items-center justify-center rounded-full bg-white/10 border border-white/10 text-slate-500 hover:text-white active:text-white transition-colors">
            <span className="material-symbols-outlined text-xl">arrow_back_ios_new</span>
          </button>
          <h1 className="text-2xl font-bold tracking-tight text-slate-500">Stardy Premium</h1>
        </div>
      </header>

      <main className="px-6 space-y-8">
        <section className="text-center py-8">
          <div className="size-24 bg-gradient-to-tr from-primary to-orange-300 rounded-[2rem] flex items-center justify-center mx-auto mb-6 shadow-2xl shadow-primary/30">

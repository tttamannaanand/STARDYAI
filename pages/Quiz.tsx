...
const Quiz: React.FC = () => {
  ...
  const questions = [
    {
      ...
      hint: "Think about how we handle things that take time, like fetching data from an API.",
    ...
    },
    {
      ...
      hint: "This hook 'effects' how the component interacts with the outside world.",
    ...
    },
    {
      ...
      hint: "The first word refers to how styles 'fall down' from one level to another.",
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-background-dark text-slate-900 dark:text-white flex flex-col transition-colors duration-500">
      <header className="bg-black px-6 pt-12 pb-6 flex flex-col gap-4 border-b border-white/10 mb-8">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/learn')} className="size-10 flex items-center justify-center rounded-full bg-white/10 border border-white/10 text-slate-500 hover:text-white active:text-white transition-colors">
              <span className="material-symbols-outlined text-xl">close</span>
            </button>
          </div>
          <div className="flex flex-col items-center">
            <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary">Tech Quiz</span>
            <h1 className="text-sm font-extrabold tracking-tight text-slate-500">Question {currentQuestion + 1} of {questions.length}</h1>
          </div>
        ...
        <div className={`flex flex-col gap-4 ${isAnswered ? 'pointer-events-none' : ''}`}>
          {q.options.map((opt) => {
            ...
            if (isAnswered) {
              ...
              buttonClass = "border-primary bg-primary/10 ring-2 ring-primary/30 shadow-lg shadow-primary/10";
        ...
        {!isAnswered ? (
          <>
            <div className="flex flex-col items-center gap-3">
              {showHint && (
                <div className="w-full p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/30 rounded-2xl animate-in fade-in slide-in-from-bottom-2 duration-300">

...
const LearningCenter: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col pb-32 bg-background-light dark:bg-background-dark">
      ...
      <header className="bg-black sticky top-0 z-50 px-6 py-4 flex flex-col gap-4 border-b border-white/10 shadow-lg">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex flex-col">
              <h1 className="text-slate-500 text-xl font-bold leading-tight">Learning Center</h1>
          ...
          <div className="flex items-center gap-2">
            <button className="size-10 rounded-full bg-white/10 flex items-center justify-center text-slate-500 hover:text-white active:text-white backdrop-blur-md">
              <span className="material-symbols-outlined text-xl">search</span>
            </button>
            <button 
              onClick={() => setIsNotifOpen(true)}
              className="size-10 rounded-full bg-white/10 flex items-center justify-center text-slate-500 hover:text-white active:text-white backdrop-blur-md"
            >
        ...
        {filteredCourses.map((course) => (
          ...
          >
            <div 
              ...
              className="bg-widget rounded-2xl overflow-hidden shadow-sm border border-slate-100 group active:scale-[0.98] transition-all cursor-pointer"
            >
              <div 
                className="h-44 w-full bg-slate-800 bg-center bg-cover relative" 
                style={{ backgroundImage: `url(${course.image})` }}
              >
                <div className="absolute top-4 left-4 px-3 py-1.5 rounded-full bg-primary text-white text-[10px] font-bold uppercase tracking-widest">
                  {course.tag}
                </div>
                {course.isNew && (
